...
public class SquadQueryRepository {
  ...
  public SquadDetailResponse fetchSquadDetail(String squadCode) {
    ...
    List<SquadDetailResponse.MemberDetail> members =
        employeeRecords.stream()
            .map(
                r ->
                    SquadDetailResponse.MemberDetail.builder()
    ...
    int estimatedDuration =
        Optional.ofNullable(squad.get(SQUAD.ESTIMATED_DURATION))
            .map(BigDecimal::intValue)
            .orElse(0);

    int totalCost =
        Optional.ofNullable(squad.get(Tables.SQUAD.ESTIMATED_COST))
            .map(BigDecimal::intValue)
            .orElse(0);
