...
public class SquadValidationService {
    ...
    public void validateSquadTitleUniqueForUpdate(String title, String projectCode, String currentSquadCode) {
        boolean exists = squadCommandRepository.existsByTitleAndProjectCodeAndSquadCodeNot(title, projectCode, currentSquadCode);
        if (exists) {
            throw new BusinessException(ErrorCode.SQUAD_TITLE_DUPLICATED);
        }
    }
    ...
    public void validateBudget(Project project, BigDecimal estimatedCost) {
        if (project.getBudget() != null &&
            ...
            throw new BusinessException(ErrorCode.EXCEED_PROJECT_BUDGET,
    ...
    public void validateJobRequirements(String projectCode, List<Developer> developers) {
        ...
        Map<Long, Long> requiredJobCounts = projections.stream()
                .collect(Collectors.toMap(
                        RequiredJobDto::getProjectAndJobId,
                        p -> Long.valueOf(p.getRequiredNumber())
                ));

        // 2. 실제 스쿼드 구성원의 직무별 인원수 집계
        Map<Long, Long> actualJobCounts = developers.stream()
                .collect(Collectors.groupingBy(
                        Developer::getProjectAndJobId,
                        Collectors.counting()
                ));
        ...
        for (Long requiredJobId : requiredJobCounts.keySet()) {
            if (!actualJobCounts.containsKey(requiredJobId)) {
                throw new BusinessException(ErrorCode.MISSING_REQUIRED_JOB,
                        "필수 직무 ID: " + requiredJobId + " 이(가) 포함되어야 합니다.");
            }
        }
        ...
        for (Map.Entry<Long, Long> entry : requiredJobCounts.entrySet()) {
            ...
            if (actual < required) {
                throw new BusinessException(ErrorCode.INSUFFICIENT_JOB_MEMBER,
    ...
    public void validateDuration(Project project, BigDecimal estimatedDuration) {
        if (project.getStartDate() != null && project.getExpectedEndDate() != null) {
            double maxMonths = ChronoUnit.MONTHS.between(
                    project.getStartDate().withDayOfMonth(1),
                    project.getExpectedEndDate().withDayOfMonth(1)) + 1;

            BigDecimal maxAllowed = BigDecimal.valueOf(maxMonths);
            if (estimatedDuration.compareTo(maxAllowed) > 0) {
                throw new BusinessException(ErrorCode.EXCEED_PROJECT_DURATION,
