...
public class MemberQueryRepository {
  ...
  public List<MemberListResponse> searchAvailableMembers(String keyword, int offset, int limit) {

    // 기술스택 서브쿼리 정의
    Table<?> topTechStack = dsl
            .select(
                    DEVELOPER_TECH_STACK.EMPLOYEE_IDENTIFICATION_NUMBER,
                    DEVELOPER_TECH_STACK.TECH_STACK_NAME,
                    rowNumber()
    ...
    Field<String> topTechStackEmpId = topTechStack.field(
            DEVELOPER_TECH_STACK.EMPLOYEE_IDENTIFICATION_NUMBER.getName(), String.class);
    Field<String> topTechStackName = topTechStack.field(
            DEVELOPER_TECH_STACK.TECH_STACK_NAME.getName(), String.class);
    Field<Integer> rowNumberField = topTechStack.field("rn", Integer.class);
    ...
    return dsl.select(
            ...
            .where(
                    MEMBER.STATUS.eq(MemberStatus.valueOf("AVAILABLE"))
                            .and(
                                    MEMBER.EMPLOYEE_IDENTIFICATION_NUMBER.containsIgnoreCase(keyword)
