...
public class DeveloperProjectWorkQueryRepository {
  ...
  private List<WorkRequestQueryDto> findByCondition(String employeeId, String status) {
    ...
    if (employeeId == null) {
      conditions.add(DEVELOPER_PROJECT_WORK.APPROVAL_STATUS.ne(DeveloperProjectWorkApprovalStatus.NOT_REQUESTED));
    }
    ...
    if (status != null && !status.isBlank()) {
      conditions.add(DEVELOPER_PROJECT_WORK.APPROVAL_STATUS.eq(DeveloperProjectWorkApprovalStatus.valueOf(status)));
    }
    ...
    var records =
            dsl.select(
                    ...
                    .on(DEVELOPER_PROJECT_WORK.DEVELOPER_PROJECT_WORK_ID.eq(
    ...
    Map<Long, List<String>> historyToStacks =
            records.stream()
                    .filter(r -> r.get(DEVELOPER_PROJECT_WORK_HISTORY.DEVELOPER_PROJECT_WORK_HISTORY_ID) != null)
    ...
    Map<Long, List<WorkRequestQueryDto.WorkRequestHistoryDto>> workToHistories =
            records.stream()
                    ...
                    .collect(Collectors.groupingBy(
                            ...
                            Collectors.mapping(
                                    r -> new WorkRequestQueryDto.WorkRequestHistoryDto(
                                            r.get(DEVELOPER_PROJECT_WORK_HISTORY.DEVELOPER_PROJECT_WORK_HISTORY_ID),
                                            r.get(DEVELOPER_PROJECT_WORK_HISTORY.FUNCTION_DESCRIPTION),
                                            historyToStacks.getOrDefault(
                                                    r.get(DEVELOPER_PROJECT_WORK_HISTORY.DEVELOPER_PROJECT_WORK_HISTORY_ID),
                                                    List.of()),
                                            Optional.ofNullable(r.get(DEVELOPER_PROJECT_WORK_HISTORY.FUNCTION_TYPE))
                                                    .map(Enum::name).orElse(null),
                                            Optional.ofNullable(r.get(DEVELOPER_PROJECT_WORK_HISTORY.COMPLEXITY))
                                                    .map(Enum::name).orElse(null)
                                    ),
                                    Collectors.toList()
                            )
