...
public class ReplacementRecommendationRepository {
  ...
  public List<DeveloperSummary> findCandidatesForReplacement(
      String projectCode, String employeeId) {
    Record1<String> squadCodeRecord =
    ...
    Record1<Long> projectAndJobIdRecord =
        dsl.select(SQUAD_EMPLOYEE.PROJECT_AND_JOB_ID)
            .from(SQUAD_EMPLOYEE)
    ...
    List<String> requiredStacks =
        dsl.select(JOB_AND_TECH_STACK.TECH_STACK_NAME)
    ...
    String domainName =
        dsl.select(PROJECT.DOMAIN_NAME)
            .from(PROJECT)
            .where(PROJECT.PROJECT_CODE.eq(projectCode))
            .fetchOne(PROJECT.DOMAIN_NAME);

    Table<?> requiredTechCount =
        dsl.select(
                DEVELOPER_TECH_STACK.EMPLOYEE_IDENTIFICATION_NUMBER.as(
            ...
            .having(
                DSL.countDistinct(DEVELOPER_TECH_STACK.TECH_STACK_NAME).eq(requiredStacks.size()))
    ...
    return dsl.select(
        ...
        .from(MEMBER)
        .join(GRADE)
        .on(MEMBER.GRADE_CODE.cast(VARCHAR).eq(GRADE.GRADE_CODE.cast(VARCHAR)))
        .join(DEVELOPER_TECH_STACK)
        .on(
            MEMBER.EMPLOYEE_IDENTIFICATION_NUMBER.eq(
                DEVELOPER_TECH_STACK.EMPLOYEE_IDENTIFICATION_NUMBER))
        .join(requiredTechCount)
        .on(
            MEMBER.EMPLOYEE_IDENTIFICATION_NUMBER.eq(
                requiredTechCount.field("EMPLOYEE_IDENTIFICATION_NUMBER", String.class)))
        .leftJoin(SQUAD_EMPLOYEE)
        .on(MEMBER.EMPLOYEE_IDENTIFICATION_NUMBER.eq(SQUAD_EMPLOYEE.EMPLOYEE_IDENTIFICATION_NUMBER))
        .leftJoin(SQUAD)
        .on(SQUAD_EMPLOYEE.SQUAD_CODE.eq(SQUAD.SQUAD_CODE))
        .leftJoin(PROJECT)
        .on(SQUAD.PROJECT_CODE.eq(PROJECT.PROJECT_CODE))
        .where(MEMBER.STATUS.eq(MemberStatus.AVAILABLE))
        .and(
            MEMBER.EMPLOYEE_IDENTIFICATION_NUMBER.notIn(
