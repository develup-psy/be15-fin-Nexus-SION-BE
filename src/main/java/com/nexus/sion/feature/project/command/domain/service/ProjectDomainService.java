...
public class ProjectDomainService {
    ...
    public ProjectAnalysisResult analyze(FPInferResponse result) {
        ...
        ProjectFpSummary summary = ProjectFpSummary.builder()
        ...
        // 기능 단위 리스트 생성
        List<ProjectFunctionEstimate> functions = result.getFunctions().stream()
                .filter(func -> isValidFunctionType(func.getFp_type()) && isValidComplexity(func.getComplexity()))
                .map(func ->
                        ProjectFunctionEstimate.builder()
                                .projectFpSummaryId(summary.getId())
                                .functionType(ProjectFunctionEstimate.FunctionType.valueOf(func.getFp_type().toUpperCase()))
                                .complexity(ProjectFunctionEstimate.Complexity.valueOf(func.getComplexity().toUpperCase()))
