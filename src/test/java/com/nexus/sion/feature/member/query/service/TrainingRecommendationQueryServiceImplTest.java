...
class TrainingRecommendationQueryServiceImplTest {
    ...
    @Mock
    private TrainingProgramQueryRepository trainingRepo;
    ...
    void recommendTrainingsFor_success() {
        ...
        // 기술 스택 점수
        given(techStackRepo.findTechStacksByEmployeeId(employeeId)).willReturn(List.of(
        ...
        // 교육 프로그램
        given(trainingRepo.findByCategory("Spring-INTERMEDIATE")).willReturn(List.of(
                new TrainingProgramResponse(1L, "Spring 중급", "중급 과정", "Spring-INTERMEDIATE", "url1", "video1")
        ));
        given(trainingRepo.findByCategory("Redis-BEGINNER")).willReturn(List.of(
                new TrainingProgramResponse(2L, "Redis 초급", "초급 과정", "Redis-BEGINNER", "url2", "video2")
        ));

        // 자격증
        given(certQueryService.findAllCertificateNames()).willReturn(List.of("정보처리기사", "SQLD"));
        given(certQueryService.findOwnedCertificateNamesByEmployee(employeeId)).willReturn(List.of("SQLD"));
        given(trainingRepo.findByCategoryIn(List.of("정보처리기사"))).willReturn(List.of(
                new TrainingProgramResponse(3L, "정보처리기사 대비", "필기/실기 준비", "정보처리기사", "url3", "video3")
        ));
