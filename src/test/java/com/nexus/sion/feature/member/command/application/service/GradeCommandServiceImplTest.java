...
class GradeCommandServiceImplTest {
    ...
    @Mock
    private GradeCommandRepository gradeRepository;

    @Mock
    private ModelMapper modelMapper;

    @InjectMocks
    private GradeCommandServiceImpl gradeCommandService;
    ...
    void setGrades_shouldCreateAndUpdateGradesCorrectly() {
        ...
        GradeDto existingGradeDto = GradeDto.builder()
                .gradeCode(GradeCode.B)
                .monthlyUnitPrice(110000)
                .build();
        ...
        UnitPriceSetRequest request = UnitPriceSetRequest.builder()
                .grades(gradeDtos)
                .build();
        ...
        when(modelMapper.map(newGradeDto, Grade.class)).thenReturn(
                Grade.builder()
                        .gradeCode(GradeCode.A)
                        .productivity(new BigDecimal(3))
                        .monthlyUnitPrice(100000)
        ...
        verify(gradeRepository).save(argThat(grade ->
                grade.getGradeCode() == GradeCode.B &&
                        grade.getProductivity().compareTo(new BigDecimal(2)) == 0 && // updated
