...
class NotificationQueryServiceTest {
    ...
    void testGetNotifications() {
        ...
        List<NotificationDTO> mockNotifications = List.of(
                new NotificationDTO(1L, NotificationType.SQUAD_COMMENT, "123L", "message1", false, LocalDateTime.now(), "sender1", "senderName1", "receiver1"),
                new NotificationDTO(2L, NotificationType.SQUAD_COMMENT, "123L", "message1", false, LocalDateTime.now(), "sender2", "senderName2", "receiver2")
        );

        given(notificationQueryRepository.selectNotifications(page, size, memberId))
                .willReturn(mockNotifications);
        given(notificationQueryRepository.countTotalNotifications(memberId))
                .willReturn(2L);
    ...
    void testGetAllNotifications() {
        ...
        given(notificationQueryRepository.selectAllNotifications(page, size))
                .willReturn(mockNotifications);
        given(notificationQueryRepository.countTotalAllNotifications())
                .willReturn(2L);
