...
class NotificationCommandIntegrationTest {
  ...
  void 스쿼드_공유_알림_API_성공() throws Exception {
    // given
    SquadShareNotificationRequest request =
        SquadShareNotificationRequest.builder()
  ...
  void 인증되지_않은_사용자_구독_요청_실패() throws Exception {
    mockMvc.perform(get("/api/v1/notifications/connect")).andExpect(status().is4xxClientError());
  }
