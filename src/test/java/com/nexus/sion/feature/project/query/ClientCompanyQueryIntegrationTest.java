...
public class ClientCompanyQueryIntegrationTest {
    ...
    void getAllClientCompanies() throws Exception {
        mockMvc.perform(get("/api/v1/client-companies")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk()).andExpect(jsonPath("$.success").value(true))
                .andExpect(jsonPath("$.data.clientCompanies[*].clientCode")
                        .value(hasItems("test1", "test2")))
                .andExpect(jsonPath("$.data.clientCompanies[*].companyName")
                        .value(hasItems("company1", "company2")))
                .andExpect(jsonPath("$.data.clientCompanies[*].domainName")
                        .value(hasItems("domain1", "domain1")));
    }
}
