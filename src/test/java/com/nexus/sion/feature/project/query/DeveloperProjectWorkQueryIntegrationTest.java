...
class DeveloperProjectWorkQueryIntegrationTest {
  ...
  void getRequestsForAdmin() throws Exception {
    mockMvc
        .perform(
            get("/api/v1/dev-project-works/admin")
  ...
  void getFunctionTypes() throws Exception {
    mockMvc
        .perform(get("/api/v1/dev-project-works/function-types"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
        .andExpect(jsonPath("$.data").isArray())
        .andExpect(
            jsonPath("$.data.length()")
                .value(DeveloperProjectWorkHistory.FunctionType.values().length))
  ...
  void getProjectHistoryDetail() throws Exception {
    mockMvc
        .perform(get("/api/v1/dev-project-works/{projectWorkId}", testWorkId))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success").value(true))
