...
class DeveloperProjectWorkCommandServiceImplTest {
  ...
  @InjectMocks private DeveloperProjectWorkCommandServiceImpl service;

  @Mock private DeveloperProjectWorkRepository workRepository;

  @Mock private DeveloperProjectWorkHistoryRepository workHistoryRepository;

  @Mock private DeveloperProjectWorkHistoryTechStackRepository workHistoryTechStackRepository;
  ...
  void approve_fail_ifNotAdmin() {
    when(memberRepository.existsByEmployeeIdentificationNumberAndRole("USER001", MemberRole.ADMIN))
        .thenReturn(false);
  ...
  void addHistories_success() throws Exception {
    ...
    WorkHistoryAddRequestDto.WorkHistoryItemDto item =
        new WorkHistoryAddRequestDto.WorkHistoryItemDto();
    ...
    verify(notificationCommandService)
        .createAndSendNotification(
            ...
            eq("ADMIN001"),
            isNull(),
            eq(NotificationType.TASK_APPROVAL_REQUEST),
            eq("10"));
  }
